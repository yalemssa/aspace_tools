...
def create_repositories(csv_row):
    ...
    '''
    new_repo = {'jsonmodel_type': 'repository', 'repo_name': csv_row['repo_name']}
    return (new_repo, '/repositories')

@atl.as_tools_logger(logger)
def create_archival_objects(csv_row):
...
@atl.as_tools_logger(logger)
def create_classification(csv_row):
...
@atl.as_tools_logger(logger)
def create_classification_term(csv_row):
...
@atl.as_tools_logger(logger)
def create_classification_scheme(csv_row):
...
@atl.as_tools_logger(logger)
def create_digital_objects(csv_row):
...
@atl.as_tools_logger(logger)
def create_digital_object_component(csv_row):
...
@atl.as_tools_logger(logger)
def create_location_profiles(csv_row):
...
@atl.as_tools_logger(logger)
def create_digital_object_instances(record_json, csv_row):
...
def create_events(csv_row):
    ...
    new_event = {'jsonmodel_type': 'event', 'event_type': csv_row['event_type'], 'outcome': csv_row['outcome'],
                ...
                'repository': {'ref': '/repositories/' + str(csv_row['repo'])},
                'linked_records': [{'ref': csv_row['record_link'], 'role': 'source'}],
                'linked_agents': [{'role': 'authorizer', 'ref': csv_row['agent']}],
                'external_documents': []}
    if csv_row['external_doc_title'] != '':
...
@atl.as_tools_logger(logger)
def create_top_containers(csv_row):
    ...
    if csv_row['barcode'] != '':
        new_top_container['barode'] = csv_row['barcode']
...
@atl.as_tools_logger(logger)
def create_instances(record_json, csv_row):
    '''Creates an instance of a top container and links to an archival object record.

       Parameters:
        record_json: The JSON representation of the archival object.
        csv_row['record_uri']: The URI of the archival object record.
...
@atl.as_tools_logger(logger)
def create_multipart_note(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def create_container_profiles(csv_row):
...
@atl.as_tools_logger(logger)
def create_hm_external_ids(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def create_file_version(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def create_file_versions(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def create_access_notes(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def create_local_access_restriction(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def create_timebound_restriction(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def update_identifiers(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def update_container_type(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def link_agent_to_record(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def link_event_to_record(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def link_record_to_classification(record_json, csv_row):
...
#abstract
@atl.as_tools_logger(logger)
def update_top_containers(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def update_dates(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def update_date_type(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def update_folder_numbers(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def update_revision_statements(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def update_notes(record_json, csv_row):
    '''Updates singlepart or multipart notes.
       ...
       Parameters:
        ...
        csv_row['uri']: The URI of the parent record.
        csv_row['persistent_id']: The persistent ID of the parent note.
        csv_row['new_text']: The new note text.
...
@atl.as_tools_logger(logger)
def update_access_notes(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def update_locations(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def update_location_coordinates(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def update_record_component(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def update_record_components(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def update_subrecord_component(record_json, csv_row):
...
#also need to define the subrecord here...
@atl.as_tools_logger(logger)
def update_subrecord_components(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def update_record_pub_status(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def update_subrecord_pub_status(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def update_note_pub_status(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def update_names(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def update_sources_auth_ids(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def delete_subcontainers(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def delete_notes(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def delete_rights_restriction(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def delete_instances(record_json, csv_row):
    '''Deletes an instance.
...
@atl.as_tools_logger(logger)
def get_series(record_json, csv_row):
...
@atl.as_tools_logger(logger)
def get_agents(agent_json, csv_row):
    '''Retrieves data about agents.

       Parameters:
        agent_json: The JSON representation of the agent record.
...
@atl.as_tools_logger(logger)
def get_subjects(subject_json, csv_row):
